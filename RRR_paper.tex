...
\affil[1]{The Program in Applied and Computational  Mathematics, Princeton University, Princeton, NJ, USA}
\affil[2]{School of Electrical Engineering, Tel Aviv University, Tel Aviv, Israel}
\maketitle
...
In the last decade, the computational and theoretical aspects of the phase retrieval problem gained much attention. 
As a starting point, most papers considered a toy model in which the Fourier measurements are replaced by inner products with random vectors; hereafter we refer to the toy model as the \emph{random phase retrieval problem}.
The random phase retrieval  problem has been proved to be  considerably easier and its importance to practical applications is dubious. 

The the state-of-the-art algorithms for the random phase retrieval problem are based on minimizing non-convex loss functions using first-order gradient algorithm; see for instance~\cite{candes2015phase,chen2017solving,wang2017solving}.
Remarkably, the line of papers derive strong theoretical guarantees by showing that the intrinsic non-convexity of the problem is usually benign. 
In practice, however, the situation is radically different; for almost all phase retrieval applications, the solutions proposed for the random phase retrieval setup simply do not work---namely, the non-convexity is not benign and gradient-based algorithms are trapped in local minima, far from the global solution.
Instead, many heuristic algorithms exist that are use in the crystallography community, including the hybrid input-output (HIO)~\cite{Fienup1982}, difference map~\cite{elser2003phase}, Relaxed averaged alternating reflections (RAAR)~\cite{Luke2005}, and relax-reflect-reflect (RRR)~\cite{Elser2017a}.  In this work, we focus in particular in the RRR algorithm as a representative example of these algorithm; we do not claim it outperforms its competitors, but use it because of its simple iterations. These algorithm enjoy good empirical performance, recovering the correct solution every time, but no theoretical guarantees are known for them. 
It is not only that their convergence is unclear, but their dynamics as well. \TODO{Now we need to provide an example.}

\TODO{About DR: ~\cite{phan2016linear,hesse2013nonconvex,lindstrom2018survey}}
For certain choices of their parameters 
all  the above algorithms coincide with with the Douglas-Rachford algorithm, although the theoretical guarantees associated with Douglas-Rachford do not apply to this problem because of its non-convexity. This subject will be discussed in detail in the next section.
The main goal of this paper is to unveil  the relation between those heuristic algorithms and the gradient-based algorithms which are very popular these days. In particular, we show that XXX
